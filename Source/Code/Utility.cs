...
namespace Cthulhu
{
    public static class Utility
    {
        ...
        public static bool TryGetUnreservedPewSpot(Thing pew, Pawn claimer, out IntVec3 loc)
        {
            ...
            if (!map.reservationManager.IsReservedByAnyoneOf(pew.Position, Faction.OfPlayer))
            {
                loc = pew.Position;
                return true;
            }
            ...
            {
                if (!map.reservationManager.IsReservedByAnyoneOf(CellWest, Faction.OfPlayer))
                {
                    loc = CellWest;
                    return true;
                }

                if (!map.reservationManager.IsReservedByAnyoneOf(CellEast, Faction.OfPlayer))
                {
                    loc = CellEast;
                    return true;
                }
            ...
            if (!map.reservationManager.IsReservedByAnyoneOf(CellNorth, Faction.OfPlayer))
            {
                loc = CellNorth;
                return true;
            }

            if (map.reservationManager.IsReservedByAnyoneOf(CellSouth, Faction.OfPlayer))
            {
                return false;
            }
        ...
        public static bool ApplySanityLoss(Pawn pawn, float sanityLoss = 0.3f, float sanityLossMax = 1.0f)
        {
            ...
            if (pawnSanityHediff != null)
            {
                ...
                DebugReport("Applied Sanity loss to: " + pawn.LabelShort);
            }
            else if (sanityLoss > 0)
            {
                ...
                appliedSuccessfully = true;
                DebugReport("Made and applied Sanity loss to: " + pawn.LabelShort);
                //Log.Message("4");
            }
